...
wave_ebmf = function(Y,Kmax=100,tol=0.01,
  ...
  # rows are wavelet coefficients. finest level comes first, the last element is the summation.
  Yw = t(apply(Y,1,haar))

  # wd.list = lapply(1:n,function(i){
  #   wd(Y[i,],family=family,filter.number=filter.number)
  # })

  # # rows are wavelet coefficients. finest level comes first
  # Yw = do.call(rbind,lapply(wd.list,function(z){z$D}))
