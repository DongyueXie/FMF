...
It seems not but one observation is that the function achieves the minimum at $\mu=0$. We can plug $\mu = 0$ to $(\log f(\mu))''$. 

I'll derive the formulas using a mixture of $K$ components for simplicity.

\[(\log f(\mu))'' = \frac{\sum_k\frac{w_k}{\sqrt{2\pi(\sigma^2_k+\sigma^2)}}e^{-\frac{\mu^2}{2(\sigma^2_k+\sigma^2)}}\left((\frac{\mu^2}{\sigma^2+\sigma^2_k})^2-\frac{1}{\sigma^2+\sigma^2_k}\right)}{\sum_k\frac{w_k}{\sqrt{2\pi(\sigma^2_k+\sigma^2)}}e^{-\frac{\mu^2}{2(\sigma^2_k+\sigma^2)}}}
\\
